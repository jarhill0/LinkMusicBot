...
class ServiceHandler:
    ...
    def link_to_object(self, link):
        ...
        return (
            self.link_to_song(link)
            if self.link_is_song(link)
            else self.link_to_album(link)
        )
    ...
    def object_to_link(self, obj):
        ...
        return (
            self.song_to_link(obj) if isinstance(obj, Song) else self.album_to_link(obj)
        )
...
class AppleMusic(ServiceHandler):
    ...
    def album_to_link(self, album):
        ...
        search = self._search(
            self._format_query(album), media="music", entity="album", explicit=True
        )
    ...
    def link_to_album(self, link):
        ...
        return Album(
            unescape(page_info["name"]), unescape(page_info["byArtist"]["name"])
        )
    ...
    def link_to_song(self, link):
        ...
        track_name = (
            page.find(class_="is-deep-linked")
            .find(class_="table__row__headline")
            .string.strip()
        )
        ...
        return Song(
            unescape(track_name),
            unescape(page_info["byArtist"]["name"]),
            unescape(page_info["name"]),
        )

    def search(self, query, n=5):
        """Perform a search with a given query. Returns a list of Song or Album objects."""
        search = self._search(
            query, media="music", entity="song,album", explicit=True, limit=n
        )
        for item in search["results"]:
            ...
            if wrapper_type == "track":
                ...
                yield Album(item["collectionName"], item["artistName"])

    def song_to_link(self, song):
        """Turn a Song into an Apple Music link."""
        search = self._search(
            self._format_query(song), media="music", entity="song", explicit=True
        )
        if search["resultCount"] < 1:
...
class Spotify(ServiceHandler):
    ...
    def _naive_query(self, item):
        ...
        return "{} {}".format(re.sub(self._PARENS_PATTERN, "", item.title), item.artist)
    ...
    def album_to_link(self, album):
        ...
        if search["albums"]["total"] < 1:
            ...
            if search["albums"]["total"] < 1:
                raise ValueError(
                    "Couldn't find any album results for {!r}.".format(album)
                )
    ...
    def song_to_link(self, song):
        ...
        if search["tracks"]["total"] < 1:
            ...
            if search["tracks"]["total"] < 1:
                raise ValueError(
                    "Couldn't find any song results for {!r}.".format(song)
                )
...
class YouTube(ServiceHandler):
    ...
    def _playlist_link(playlist_data):
        ...
        return "https://www.youtube.com/playlist?list={}".format(
            playlist_data["id"]["playlistId"]
        )
    ...
    def can_handle_link(self, link):
        ...
        return urlparse(link).netloc.lower() in (
            "youtube.com",
            "www.youtube.com",
            "m.youtube.com",
            "youtu.be",
    ...
    def link_to_album(self, link):
        ...
        unexpected behavior!
        """
        params = {"part": "snippet", "id": self._link_to_id(link)}
        ...
        )  # empty artist because we really can't know
